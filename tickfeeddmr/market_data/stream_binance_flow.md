...
`stream_binance` вообще не трогает Postgres после старта (только один раз
читает список активных пар). `consume_market_data_stream` вообще не знает
...
Без неё `stream_binance` не найдёт, что стримить, и завершится сразу после
старта (см. шаг 3 ниже). `is_active` — это «собираем ли мы сейчас данные»,
...
2. `_run()` идёт в Postgres: `CryptoAsset.objects.filter(exchange=EXCHANGE, is_active=True).values_list("trading_pair", ...)`. Пример результата: `["BTCUSDT"]`.
3. Если список пуст — лог warning, `return`, процесс завершается, сокет даже не открывается.
4. Создаётся `BinanceProvider` — хранит адреса REST/WS из настроек (`BINANCE_REST_BASE_URL`, `BINANCE_WS_BASE_URL`), сам по себе ничего не делает.
5. Создаётся клиент Redis (`Redis.from_url(settings.REDIS_URL, decode_responses=True)`).
6. `async for event in provider.stream(trading_pairs):` — вот здесь впервые открывается сетевое соединение: **один** WebSocket на **все** пары сразу, URL вида `wss://stream.binance.com:9443/stream?streams=btcusdt@aggTrade`.
...
3 пар нет → сразу вышли / пары есть → идём дальше
4-5 подготовили provider и redis-клиент
...
7. **Лимиты Binance WS:**
   - 5 control-сообщений/сек — соблюдён неявно: после коннекта не шлём `subscribe`/`unsubscribe`, PING/PONG на уровне библиотеки `websockets`.
   - 1024 стрима на соединение — **не проверяется**: все активные пары уходят в один URL без cap/проверки количества.
